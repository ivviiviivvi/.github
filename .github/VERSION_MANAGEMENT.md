...
This document describes the centralized version management system for Python, Node.js, and GitHub Action dependencies across workflows.
...
2. **Composite Actions** - Standardized setup with fallback logic
3. **Auto-Update Workflows** - Automated version detection and PR creation
...
2. `vars.PYTHON_VERSION_DEFAULT` repository variable
3. Hardcoded default (`3.12`)
...
2. `vars.NODE_VERSION_DEFAULT` repository variable
3. Hardcoded default (`20`)

## Reusable Workflows

The reusable workflows in `.github/workflows/reusable/` also support the fallback pattern:
...
Workflows can have metadata sidecars following the FUNCTIONcalled naming convention. This provides structured classification and enables registry generation.
...
The workflow registry at `registry/workflow-registry.json` catalogs all workflows with:
...
2. **Or use the composite action:**
   ```yaml
   # Replace direct setup-python call
   - uses: ./.github/actions/setup-python-standard
   ```

3. **Add version determination step (for complex cases):**
   ```yaml
...
2. Use the light profile for quick adoption
3. Optionally upgrade to full profile with FUNCTIONcalled classification
...
2. Is the repository variable defined in Settings?
3. Is the hardcoded default appropriate?
...
3. Check workflow logs for validation failures

### Metadata Validation Failing

1. Validate JSON syntax: `python -m json.tool file.meta.json`
2. Check required fields for your profile level
3. Verify UUID format: `urn:uuid:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`
