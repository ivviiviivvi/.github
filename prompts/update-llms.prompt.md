...
Update the existing `llms.txt` file in the root of the repository to reflect
changes in documentation, specifications, or repository structure. This file
provides high-level guidance to large language models (LLMs) on where to find
relevant content for understanding the repository's purpose and specifications.

## Primary Directive

Update the existing `llms.txt` file to maintain accuracy and compliance with the
llms.txt specification while reflecting current repository structure and
content. The file must remain optimized for LLM consumption while staying
human-readable.
...
- Review the official specification at https://llmstxt.org/ to ensure continued
  compliance
...
The updated `llms.txt` file must maintain this exact structure per the
specification:

1. **H1 Header**: Single line with repository/project name (required)
1. **Blockquote Summary**: Brief description in blockquote format (optional but
   recommended)
1. **Additional Details**: Zero or more markdown sections without headings for
   context
1. **File List Sections**: Zero or more H2 sections containing markdown lists of
   links
...
- **Optional**: Secondary files (special meaning - can be skipped for shorter
  context)
...
1. Examine the current repository structure completely
1. Compare existing file references with actual repository content
1. Identify outdated, missing, or incorrect references
1. Note any structural issues with the current file

### Step 2: Content Planning

1. Determine if the primary purpose statement needs updates
1. Review and update the summary blockquote if needed
1. Plan additions for new files and directories
1. Plan removals for outdated or moved content
1. Reorganize sections if needed for better clarity

### Step 3: File Updates

1. Update the existing `llms.txt` file in the repository root
1. Maintain compliance with the exact format specification
1. Add new file references with appropriate descriptions
1. Remove or update outdated references
...
1. Ensure the file still serves as an effective LLM navigation tool
1. Confirm the file remains both human and machine readable
...
1. Create clear, descriptive names for links
1. Write concise but informative descriptions
1. Maintain alphabetical or logical ordering within sections
1. Consider if new sections are needed for new content types
...
1. Check if relocated files should be updated rather than removed
1. Remove entire sections if they become empty
...
1. Keep essential documentation in primary sections
1. Move secondary content to "Optional" section if appropriate
1. Ensure new organization improves LLM navigation
...
1. Maintain compliance with the llms.txt specification
1. Provide clear navigation to essential documentation
1. Remove outdated or incorrect references
1. Include new important files and documentation
1. Maintain logical organization for easy LLM consumption
1. Use clear, unambiguous language throughout
1. Continue to serve both human and machine readers effectively
