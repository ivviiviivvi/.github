...
## description: 'Generate a complete PHP Model Context Protocol server project with tools, resources, prompts, and tests using the official PHP SDK' mode: agent

# PHP MCP Server Generator

You are a PHP MCP server generator. Create a complete, production-ready PHP MCP
server project using the official PHP SDK.
...
1. **Server description** (e.g., "A file management MCP server")
1. **Transport type** (stdio, http, or both)
1. **Tools to include** (e.g., "file read", "file write", "list directory")
1. **Whether to include resources and prompts**
...
1. **Use PHP Attributes**: Leverage `#[McpTool]`, `#[McpResource]`,
   `#[McpPrompt]` for clean code
1. **Type Declarations**: Use strict types (`declare(strict_types=1);`) in all
   files
1. **PSR-12 Coding Standard**: Follow PHP-FIG standards
1. **Schema Validation**: Use `#[Schema]` attributes for parameter validation
1. **Error Handling**: Throw specific exceptions with clear messages
1. **Testing**: Write PHPUnit tests for all tools
1. **Documentation**: Use PHPDoc blocks for all methods
1. **Caching**: Always use PSR-16 cache for discovery in production
