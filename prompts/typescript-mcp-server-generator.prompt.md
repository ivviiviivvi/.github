...
## mode: 'agent' description: 'Generate a complete MCP server project in TypeScript with tools, resources, and proper configuration'

# Generate TypeScript MCP Server

Create a complete Model Context Protocol (MCP) server in TypeScript with the
following specifications:

## Requirements

1. **Project Structure**: Create a new TypeScript/Node.js project with proper
   directory structure
1. **NPM Packages**: Include @modelcontextprotocol/sdk, zod@3, and either
   express (for HTTP) or stdio support
1. **TypeScript Configuration**: Proper tsconfig.json with ES modules support
1. **Server Type**: Choose between HTTP (with Streamable HTTP transport) or
   stdio-based server
1. **Tools**: Create at least one useful tool with proper schema validation
1. **Error Handling**: Include comprehensive error handling and validation
...
- Install dependencies: `@modelcontextprotocol/sdk`, `zod@3`, and
...
- Choose appropriate transport (StreamableHTTPServerTransport or
...
- Add resources using `registerResource()` with ResourceTemplate for dynamic
  URIs
...
Generate a complete, production-ready MCP server with comprehensive
documentation, type safety, and error handling.
