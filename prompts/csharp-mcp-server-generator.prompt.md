...
## mode: 'agent' description: 'Generate a complete MCP server project in C# with tools, prompts, and proper configuration'
...
1. **Project Structure**: Create a new C# console application with proper
   directory structure
1. **NuGet Packages**: Include ModelContextProtocol (prerelease) and
   Microsoft.Extensions.Hosting
1. **Logging Configuration**: Configure all logs to stderr to avoid interfering
   with stdio transport
1. **Server Setup**: Use the Host builder pattern with proper DI configuration
1. **Tools**: Create at least one useful tool with proper attributes and
   descriptions
1. **Error Handling**: Include proper error handling and validation
