...
## mode: 'agent' description: 'Generate a complete MCP server project in Python with tools, resources, and proper configuration'

# Generate Python MCP Server

Create a complete Model Context Protocol (MCP) server in Python with the
following specifications:

## Requirements

1. **Project Structure**: Create a new Python project with proper structure
   using uv
1. **Dependencies**: Include mcp\[cli\] package with uv
1. **Transport Type**: Choose between stdio (for local) or streamable-http (for
   remote)
1. **Tools**: Create at least one useful tool with proper type hints
...
Generate a complete, production-ready MCP server with type safety, proper error
