...
## description: 'Guidelines for generating SQL statements and stored procedures' applyTo: '\*\*/\*.sql'
...
- all tables should have a column named `created_at` to store the creation
  timestamp
- all tables should have a column named `updated_at` to store the last update
  timestamp
...
- all foreign key constraints should reference the primary key of the parent
  table
...
- organize clauses consistently (SELECT, FROM, JOIN, WHERE, GROUP BY, HAVING,
  ORDER BY)
