...
## description: 'Terraform conventions and guidelines for SAP Business Technology Platform (SAP BTP).' applyTo: '\*\*/_.tf, \*\*/_.tfvars, \*\*/_.tflint.hcl, \*\*/_.tf.json, \*\*/\*.tfvars.json'
...
1. Add it as a tool in your Copilot / MCP client configuration.
1. Query provider schema (e.g., list resources, data sources) before authoring.
1. Generate draft resource blocks, then refine manually for naming & tagging
   standards.
1. Validate plan summaries (never include secrets); confirm diff with reviewer
   before `apply`.
