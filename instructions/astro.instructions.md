...
## description: 'Astro development standards and best practices for content-driven websites' applyTo: '\*\*/_.astro, \*\*/_.ts, \*\*/_.js, \*\*/_.md, \*\*/\*.mdx'

# Astro Development Instructions

Instructions for building high-quality Astro applications following the
content-driven, server-first architecture with modern best practices.
...
- Static site generation (SSG) by default with optional server-side rendering
  (SSR)
...
- Embrace the Islands Architecture: server-render by default, hydrate
  selectively
- Organize content with Content Collections for type-safe Markdown/MDX
  management
...
- Import framework components (React, Vue, Svelte) only when interactivity is
  needed
...
- Use built-in loaders: `glob()` for file-based content, `file()` for single
  files
...
- Legacy `type: 'content'` collections still supported via automatic glob()
  implementation
...
- Enable with `<ClientRouter />` component in layout head (renamed from
  `<ViewTransitions />` in v5.0)
...
- **Content Layer API**: New `glob()` and `file()` loaders for enhanced
  performance
...
1. Configure Content Layer API with appropriate loaders
1. Set up TypeScript with `astro sync` for type generation
1. Create layout components with Islands Architecture
1. Implement content pages with SEO and performance optimization

### Astro-Specific Best Practices

- **Islands Architecture**: Server-first with selective hydration using client
  directives
- **Content Layer API**: Use `glob()` and `file()` loaders for scalable content
  management
- **Zero JavaScript**: Default to static rendering, add interactivity only when
  needed
...
- **Performance**: Optimize with built-in image optimization and minimal client
  bundles
