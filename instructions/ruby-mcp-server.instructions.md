...
## description: 'Best practices and patterns for building Model Context Protocol (MCP) servers in Ruby using the official MCP Ruby SDK gem.' applyTo: "\*\*/_.rb, \*\*/Gemfile, \*\*/_.gemspec, \*\*/Rakefile"

# Ruby MCP Server Development Guidelines

When building MCP servers in Ruby, follow these best practices and patterns
using the official Ruby SDK.
...
1. **Define input/output schemas** - Ensure type safety and validation
1. **Add annotations** - Help clients understand tool behavior
1. **Include structured content** - Provide both text and structured data
1. **Use server_context** - Pass authentication and request context
1. **Configure exception reporting** - Monitor errors in production
1. **Implement instrumentation** - Track performance metrics
1. **Send notifications** - Keep clients updated on changes
1. **Validate inputs** - Check parameters before processing
1. **Follow Ruby conventions** - Use snake_case, proper indentation
