...
## applyTo: \['\*'\] description: "Comprehensive best practices for adopting new Java 25 features since the release of Java 21."

# Java 21 to Java 25 Upgrade Guide

These instructions help GitHub Copilot assist developers in upgrading Java
projects from JDK 21 to JDK 25, focusing on new language features, API changes,
and best practices.
...
- Suggest using primitive type patterns in switch expressions and instanceof
  checks
...
1. **Update Dependencies**: Check for JDK 25 compatibility
1. **Handle Warnings**: Address deprecation warnings from JEPs 471/472
1. **Enable Preview Features**: If using pattern matching or other preview
   features
1. **Test Thoroughly**: Especially for applications using JNI or sun.misc.Unsafe
1. **Performance Testing**: Verify GC behavior with new ZGC defaults
...
1. **Preview Feature Dependencies**: Don't use preview features in library code
   without clear documentation
1. **Native Access**: Applications using JNI directly or indirectly may need
   `--enable-native-access` configuration
1. **Unsafe Migration**: Don't delay migrating from sun.misc.Unsafe -
   deprecation warnings indicate future removal
1. **Pattern Matching Scope**: Primitive patterns work with all primitive types,
   not just int
1. **Record Enhancement**: Derived record creation requires preview flag in Java
   23
...
Remember to test thoroughly in staging environments before deploying Java 25
